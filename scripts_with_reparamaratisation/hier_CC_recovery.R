...
for (t in 1:ntypes) {
  for (g in 1:ngroups) {
    for (s in 1:nsub) {
      ...
      sigma_alpha <- runif(1, 0.1, 1)
      sigma_rho <- runif(1, 0.1, 1)
      ...
      # the following returns an error, as there is an incorrect number of subscripts
      # dim(samples$BUGSoutput$sims.list$mu_alpha) = 12000  2
