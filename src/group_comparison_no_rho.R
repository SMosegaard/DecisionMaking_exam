...
writeLines(output, con = "../jags_output/comparison_summary_NO_RHO.txt")

# Plotting!
source("plot_functions.R")

df <- data.frame(parameter = rep(c("alpha"),
                                 each = length(samples$BUGSoutput$sims.list$diff_alpha)),
                 samples = c(as.vector(samples$BUGSoutput$sims.list$diff_alpha)))

pl1 <- diff_plot(df)
ggsave("../plots/post_dens_diff_means_NO_RHO.png", plot = pl1, width = 6, height = 6, dpi = 300)


# diff_alpha
png("../plots/post_dens_diff_means_NO_RHO2.png", width = 800, height = 600)
...
png("../plots/traceplot_group_comparison_NO_RHO.png", width = 800, height = 600)
traceplot(samples, mfrow = c(2, 1))
dev.off()
